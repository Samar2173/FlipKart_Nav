...
class A_Star:
    ...
    def checkPoint(self):
        check_1 = (self.start[0], self.goal[1])
        check_2 = (self.goal[0], self.start[1])

        distCheck1 = self.euclidean(self.arrayCord(check_1), check_1)
        distCheck2 = self.euclidean(self.arrayCord(check_2), check_2)

        if distCheck1 < distCheck2:
            return self.arrayCord(check_1)
        
        return self.arrayCord(check_2)
    ...
    def hVal(self):
        last = self.pathList[-1]
        # print(last, self.check, self.goal)
        ...
        # print(last, goal)
        neighbors_list = self.neighbors(last)
        # print(neighbors_list)
        h_dict = {}
        for i in neighbors_list:
            dist = self.euclidean(i, goal)
            h_dict[i] = dist
    ...
    def pathCalc(self):
        self.g += 1
        self.h = self.hVal()
        if self.h == 0 and self.pathDict[self.g] == self.goal:
            print(self.pathDict[self.g])
            # print(self.h)
            return False
        elif self.g > 1:
